...
const Hero = () => {
  ...
  return (
    <div className="font-sans bg-white text-gray-900 overflow-x-hidden">
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/97 backdrop-blur-md shadow-[0_1px_16px_rgba(124,58,237,0.08)] border-b border-purple-100">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            {["preview", "features", "event-flow"].map((s) => (
              <button
                key={s}
                onClick={() => scrollTo(s)}
                className="text-sm font-medium capitalize text-gray-500 hover:text-purple-600 transition-colors"
              >
                {s.replace(/-/g, " ")}
              </button>
            ))}
          ...
          <div className="hidden md:flex items-center gap-3">
            <Link
              to="/login"
              className="text-sm font-semibold px-4 py-2 rounded-full text-purple-600 hover:bg-purple-50 transition"
            >
              Login
            </Link>
            <Link
              to="/signup"
              className="text-sm font-semibold text-white px-5 py-2 rounded-full bg-linear-to-br from-blue-900 to-purple-700 hover:opacity-90 hover:scale-105 transition"
            >
              Sign Up
            </Link>
          </div>
          ...
          <div className="md:hidden bg-white border-t border-purple-100 px-6 py-4 flex flex-col gap-4">
            {["preview", "features", "event-flow"].map((s) => (
              <button
                key={s}
                onClick={() => scrollTo(s)}
                className="text-left text-sm font-medium capitalize text-gray-700"
              >
                {s.replace(/-/g, " ")}
              </button>
            ))}
            <div className="flex gap-3 pt-2">
              <Link
                to="/login"
                className="flex-1 text-center border border-purple-600 rounded-full py-2 text-sm font-semibold text-purple-600"
              >
                Login
              </Link>
              <Link
                to="/signup"
                className="flex-1 text-center rounded-full py-2 text-sm font-semibold text-white bg-linear-to-br from-blue-900 to-purple-700"
              >
                Sign Up
              </Link>
            </div>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-linear-to-br from-purple-50 via-violet-100 to-indigo-100">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 py-32 grid md:grid-cols-2 gap-16 items-center">
          <div>
            <Logo size="lg" />
            <p className="text-2xl md:text-3xl font-semibold mb-4 text-purple-900">
            ...
            <div className="flex flex-wrap gap-4">
              <Link
                to="/signup"
                className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full font-bold text-white text-sm bg-blue-900 hover:scale-105 hover:shadow-lg transition"
              >
                Get Started <ArrowRight size={16} />
              </Link>
              <button
                onClick={() => scrollTo("features")}
                className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full font-bold text-sm bg-white border border-purple-600 text-purple-600 shadow-sm hover:scale-105 transition"
              >
          ...
          <div className="hidden md:flex justify-center items-center">
            <div className="relative w-80 h-80">
              <div
                className="absolute inset-0 rounded-full border-2 border-dashed border-purple-400 opacity-40 animate-spin"
                style={{ animationDuration: "20s" }}
              />
              <div
                className="absolute inset-8 rounded-full border-2 border-dashed border-blue-900 opacity-25 animate-spin"
                style={{
              ...
              {[
                ...
                return (
                  <div
                    key={angle}
                    className="absolute w-10 h-10 rounded-xl flex items-center justify-center bg-white shadow-[0_4px_16px_rgba(124,58,237,0.15)] border border-purple-100"
                    style={{
                      left: `${x}%`,
                      top: `${y}%`,
      ...
      <section id="event-flow" className="py-24 px-6 bg-purple-50">
        <div className="max-w-6xl mx-auto">
          <FadeIn className="text-center mb-16">
            <h2 className="text-4xl font-extrabold mt-2 text-blue-900">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              },
              {
                ...
                title: "Student",
                items: [
                  "Joins and registers for events",
                  "Earns badges and awards",
              ...
              <FadeIn key={title}>
                <div className="rounded-2xl p-6 h-full bg-white shadow-[0_2px_16px_rgba(124,58,237,0.07)] border border-purple-100">
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-4 bg-linear-to-br from-blue-900 to-purple-700">
      ...
      <footer className="py-12 px-6 bg-[#1E1B4B]">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-10">
            <div>
              <Logo />
              <p className="text-sm mt-3 leading-relaxed text-white/45">
