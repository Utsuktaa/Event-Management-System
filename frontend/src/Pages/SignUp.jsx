...
const SignUp = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isValidPassword(formData.password)) {
      alert("8 characters with uppercase, lowercase, and a number.");
      return;
    }
    try {
  ...
  const inputCls =
    "w-full px-4 py-2.5 rounded-xl text-sm text-gray-800 bg-white placeholder-gray-400 outline-none transition border border-purple-200 focus:border-purple-500 shadow-sm";

  return (
    <div className="min-h-screen flex items-center justify-center font-sans relative overflow-hidden bg-linear-to-br from-violet-100 via-purple-50 to-indigo-100">
      ...
      <div className="relative w-full max-w-md mx-4 rounded-3xl p-8 bg-white/75 backdrop-blur-xl border border-purple-200/50 shadow-[0_8px_40px_rgba(124,58,237,0.12)]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full py-2.5 rounded-xl text-sm font-semibold text-white bg-linear-to-br from-blue-900 to-purple-700 hover:opacity-90 hover:scale-[1.01] active:scale-[0.99] transition shadow-[0_4px_14px_rgba(124,58,237,0.35)]"
          >
            Sign Up
          </button>
        </form>
