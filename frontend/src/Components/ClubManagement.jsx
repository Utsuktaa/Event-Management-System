...
export default function ClubManagement({
  ...
  const [joinPolicy, setJoinPolicy] = useState(
    initialJoinPolicy || "APPROVAL_REQUIRED",
  );
  ...
  const savePolicy = async () => {
    ...
    try {
      await axios.patch(
        `http://localhost:5000/api/clubs/${clubId}/policy`,
        { joinPolicy },
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  return (
    <div className="max-w-4xl mx-auto mt-4">
          ...
          <div className="flex-1 rounded-2xl p-6" style={cardStyle}>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-3">
              <input
                ...
                required
                className="w-full px-4 py-2.5 rounded-xl text-sm placeholder-gray-400 outline-none transition"
                style={inputStyle}
              ...
              <textarea
                ...
                className="w-full px-4 py-2.5 rounded-xl text-sm placeholder-gray-400 outline-none resize-none transition"
                style={inputStyle}
                onFocus={(e) => (e.target.style.borderColor = "#7C3AED")}
                onBlur={(e) =>
                  (e.target.style.borderColor = "rgba(124,58,237,0.2)")
              ...
              {editingEventId && (
                <button
                  type="button"
                  onClick={resetForm}
                  className="w-full py-2 rounded-xl text-sm font-medium transition hover:bg-red-50"
                  style={{
      ...
      {activeSection === "settings" && can("manage_members") && (
        <div className="max-w-lg rounded-2xl p-6" style={cardStyle}>
